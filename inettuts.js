...
var iNettuts = {
    ...
    settings : {
        ...
        widgetDefault : {
            ...
            collapsible: true,
            editable: true,
            colorClasses : ['color-yellow', 'color-red', 'color-blue', 'color-white', 'color-orange', 'color-green', 'color-pink']
        },
        widgetIndividual : {}
    },
    ...
    addWidgetControls : function () {
        ...
        $(settings.widgetSelector, $(settings.columns)).each(function () {
            ...
            if (thisWidgetSettings.editable) {
                ...
                $('<div class="edit-box" style="display:none;"/>')
                    ...
                    .append((function(){
                        var colorList = '<li class="item"><label>Cor:</label><ul class="colors">';
                        $(thisWidgetSettings.colorClasses).each(function () {
                            colorList += '<li class="' + this + '"/>';
                        });
                        return colorList + '</ul>';
                    })())
                    .append('<li class="item"><label>Texto:</label><textarea>'+ $('p',this).text() + '"></textarea></li></ul>')
        ...
        $('.edit-box').each(function () {
            $('input',this).keyup(function () {
                $(this).parents(settings.widgetSelector).find('h3').text( $(this).val().length>58 ? $(this).val().substr(0,58)+'...' : $(this).val() );
                iNettuts.savePreferences();
            });
    ...
    savePreferences : function () {
        ...
        /* Assemble the cookie string */
        $(settings.columns).each(function(i){
            cookieString2 += (i===0) ? '' : 'array(';
            $(settings.widgetSelector,this).each(function(i){
                cookieString2 += (i===0) ? '' : ';';
                /* ID of widget: */
                cookieString2 += $(this).attr('id') + ',';
                /* Color of widget (color classes) */
                cookieString2 += $(this).attr('class').match(/\bcolor-[\w]{1,}\b/) + ',';
                /* Title of widget (replaced used characters) */
                cookieString2 += $('h3:eq(0)',this).text().replace(/\|/g,'[-PIPE-]').replace(/,/g,'[-COMMA-]') + ',';
                /* Text of widget (replaced used characters) */
                cookieString2 += $('p',this).text();
                /* Collapsed/not collapsed widget? : */
                cookieString2 += $(settings.contentSelector,this).css('display') === 'none' ? 'collapsed' : 'not-collapsed';
            });
        });
        alert(cookieString2);
        $.cookie(settings.saveToCookie2,cookieString2,{
            expires: 10
            //path: '/'
        });
    },
    ...
    sortWidgets : function () {
        ...
        $(settings.columns).each(function(i){
            ...
            $(widgetData).each(function(){
                ...
                $(clonedWidget).find('p').html(thisWidgetData[3]);
